...
terrain = COLOR(BROWN2)(MAP(mounts)(domainTerrain))
...
def settlement (w, l, h) :
	def set0 (subset) :
		...
		horizontal_road = CUBOID([x2-x1, road_width, 0])

		vertical_road = CUBOID([road_width, y2-y1, 0])

		for x in np.arange(x1, x2, w + 0.05 + road_width + 0.05):
			for y in np.arange(y1, y2, l + 0.05 + road_width + 0.05):
				if random.random() > 0.25 :	
					b = building(w, l, h)
					buildings.append(T([1, 2, 3])([x, y, z1])(b))

				buildings.append(T([1, 2, 3])([x1, y + 0.05 + road_width, z1])(horizontal_road))
			
			buildings.append(T([1, 2, 3])([x + 0.05 + road_width + 0.05, y1, z1])(vertical_road))
...
settlement1 = settlement(0.1, 0.1, 0.2)([PI/10, 3*PI/4, PI/20, 2*PI/5, 0.02])
